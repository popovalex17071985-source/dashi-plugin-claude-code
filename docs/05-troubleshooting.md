...
## Проблема 13. Бот молчит в групповом чате (в личке отвечает)

### Симптом

В личке агент отвечает нормально. В группе — тишина на обычные сообщения;
иногда отвечает, только если написать `@его_username` или команду со слэшем.
Логи агента чистые: входящего сообщения в них просто нет.

### Корень

Telegram **privacy mode** включён у ботов по умолчанию. Бот со статусом
`member` НЕ получает обычные сообщения группы — до него доходят только
`/команды`, прямые @упоминания и reply на его собственные сообщения.
Это отсечка на стороне Telegram: никакой код агента её не увидит.

### Диагностика — 10 секунд, ДО любого разбора кода

Любым ботом, который уже состоит в этой группе:

```bash
# id бота по username
curl -s "https://api.telegram.org/bot$TOKEN/getChat?chat_id=@TargetBot" | jq '.result.id'
# его статус в группе
curl -s "https://api.telegram.org/bot$TOKEN/getChatMember?chat_id=<CHAT_ID>&user_id=<BOT_ID>" \
  | jq '.result.status'
```

`"administrator"` — privacy не действует, копать дальше в агенте.
`"member"` — вот и причина, дальше не копать.

### Фикс

Сделать бота администратором группы: настройки группы → участники → бот →
администратор. Права выдавать не обязательно, значение имеет сам статус.
Это делает любой админ группы, владелец бота не нужен.

Альтернатива на стороне владельца бота: BotFather → `/setprivacy` → Disable,
после чего бота нужно удалить из группы и добавить заново.

### Как не повторить

При заведении агента в групповой чат — сразу админом, тем же движением, что
и добавление. Симптом «молчит в группе» проверять с прав, а не с кода:
10.09.2026 на этом потеряли день, разбирая исправный выходной путь агента.
