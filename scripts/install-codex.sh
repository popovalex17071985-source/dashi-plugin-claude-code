...
──────────────────────────────────────────────────────────────
  ...
  ВАЖНО: логин заканчивается возвратом на localhost СЕРВЕРА, поэтому
  подключись с компьютера с пробросом порта (иначе «Route Error» в браузере):

  1) На своём компьютере, в НОВОМ окне терминала:
       ssh -L 1455:localhost:1455 root@$(hostname -I 2>/dev/null | awk '{print $1}')
  2) В этом окне:  $CODEX_BIN login
  3) Codex покажет ССЫЛКУ — склей её в одну строку без пробелов,
     открой в браузере НА ЭТОМ ЖЕ компьютере, войди в аккаунт ChatGPT
     (тот, где оплачена подписка).
  4) В консоли появится «signed in».
  5) Снова: bash $0
