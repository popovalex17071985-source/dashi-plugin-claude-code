...
──────────────────────────────────────────────────────────────
  ...
  1) $CODEX_BIN login
     (полный путь — текущая ssh-сессия может ещё не видеть новый PATH)
  2) Codex покажет ССЫЛКУ и КОД. Терминал может разорвать ссылку
     на несколько строк — склей её в одну без пробелов.
  3) Открой ссылку на телефоне/компе, войди в аккаунт ChatGPT
     (тот, где оплачена подписка), введи код.
