...
# Своя AGENTS.md (характер переписан вручную) могла потерять раздел памяти -- тогда
# агент ничего не запоминает. Характер не трогаем, раздел дописываем в конец (с бэкапом).
...
- Я написал «запомни ...» — сохрани суть в memory/<тема>.md (создай или дополни),
  ...
  память в голове пропадает с новой сессией, остаётся только файл.
...
if [[ -f "$CODEX_HOME/AGENTS.md" ]]; then
  if grep -q "ТОЛЬКО после того, как команда записи" "$CODEX_HOME/AGENTS.md"; then
    skip "AGENTS.md на месте (характер не трогаю)"
  elif grep -q "MEMORY.md" "$CODEX_HOME/AGENTS.md"; then
    # Старый раздел памяти: агент говорил «запомнил», ничего не записав. Вставляем правило.
    cp "$CODEX_HOME/AGENTS.md" "$CODEX_HOME/AGENTS.md.bak-$(date +%Y%m%d%H%M%S)"
    sed -i '/подтверди мне, что записал\./a\  «Запомнил» пиши ТОЛЬКО после того, как команда записи в файл выполнилась —\n  память в голове пропадает с новой сессией, остаётся только файл.' "$CODEX_HOME/AGENTS.md"
    ok "AGENTS.md: память -- «запомнил» только после записи в файл"
  else
    cp "$CODEX_HOME/AGENTS.md" "$CODEX_HOME/AGENTS.md.bak-$(date +%Y%m%d%H%M%S)"
    printf '\n\n%s\n' "$MEMORY_SECTION" >> "$CODEX_HOME/AGENTS.md"
    ok "AGENTS.md: дописал раздел памяти (характер не тронут, бэкап рядом)"
  fi
else
