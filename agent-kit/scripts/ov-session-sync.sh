...
TEMP_FILE_ID="$(echo "$UPLOAD_RESPONSE" | jq -r '.result.temp_file_id // .temp_file_id // empty')"
...
RESOURCE_URI="viking://resources/${AGENT_NAME}-sessions/${DATE}.md"
...
OUTER_STATUS="$(echo "$ADD_RESPONSE" | jq -r '.status // "unknown"')"
INNER_STATUS="$(echo "$ADD_RESPONSE" | jq -r '.result.status // empty')"
ERRORS="$(echo "$ADD_RESPONSE" | jq -r '.result.errors // .error // empty')"
if [ "$OUTER_STATUS" = "ok" ] && { [ "$INNER_STATUS" = "success" ] || [ -z "$INNER_STATUS" ]; }; then
  echo "$LOG_PREFIX add_resource -> $RESOURCE_URI: ok${INNER_STATUS:+ ($INNER_STATUS)}"
else
  echo "$LOG_PREFIX ERROR: add_resource failed: $ADD_RESPONSE" >&2
  exit 1
fi
