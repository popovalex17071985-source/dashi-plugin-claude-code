#!/usr/bin/env python3
"""Shared helpers for the OpenViking memory hooks (recall + digest capture).
...
OV_AGENT = "__AGENT__"
OV_TIMEOUT_S = 6
STATE_FILE = Path("__WORKSPACE__") / "data/ov-capture-state.json"
LOG_FILE = Path("__WORKSPACE__") / "logs/ov-memory.log"

RECALL_LIMIT = 3
RECALL_THRESHOLD = 0.40  # measured 2026-08-21: relevant 0.39-0.51, noise mostly <0.40 on clean queries
MIN_QUERY_WORDS = 3
MAX_ASK_CHARS = 500
...
class Digest:
    ...
    def user_text(self) -> str:
        ...
        return (f"{tag}Запись от {now} (новый эпизод): {_trim(self.answer, MAX_ANSWER_CHARS)} "
                f"Повод: Саня спросил/поручил «{ask}»")
...
class OVClient:
    def __init__(self, base: str = OV_BASE, agent: str = OV_AGENT, timeout: float = OV_TIMEOUT_S):
        ...
        # The server routes memory by X-OpenViking-User. The old header name
        # ("...-Agent") is unknown to it, so EVERY agent wrote into the shared
        # «default» user -- a fresh agent read the coordinator's episodes and
        # dumped its own on top of them (verified against the live API 19.09.2026).
        self.headers = {"Content-Type": "application/json", "X-OpenViking-User": agent}

    def call(self, path: str, body=None, method: str | None = None, timeout: float | None = None):
    ...
    def store_digest(self, digest: Digest) -> list[str]:
        ...
        try:
            ...
            out = self.call(f"/api/v1/sessions/{sid}/extract", {}, timeout=90) or []
...
def log(event: str, **kw) -> None:
    try:
        ...
        # ISO timestamp prefix (local tz) — the JSON payload after it is unchanged.
        stamp = datetime.now(ZoneInfo("Asia/Yekaterinburg")).isoformat(timespec="seconds")
        with LOG_FILE.open("a", encoding="utf-8") as fh:
            fh.write(stamp + " " + json.dumps({"event": event, **kw}, ensure_ascii=False) + "\n")
    except OSError:
        pass
