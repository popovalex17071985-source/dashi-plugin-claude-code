...
Класс ошибки: правлю код и в том же ходе пишу Сане «поправил / работает», хотя
...
обрывались на первой позиции, увидел это Саня, не я. Harness Engineering
...
сообщении Сане есть заявление о результате и (3) после последней правки не было
...
def is_prompt(e: dict) -> bool:
    """Настоящий ввод Сани: не tool_result и не врезка хука.
...
def scan(entries: list[dict]) -> tuple[str, int, int]:
    """(текст Сане; позиция последней правки кода; позиция последнего прогона).
...
def verdict(text: str, last_edit: int, last_run: int) -> str | None:
    ...
    if last_run > last_edit:
        return None
    return (
        "Ты правил код и отчитываешься Сане о результате, но после последней "
        ...
        "непроверенным, ошибку нашёл Саня.\n"
