#!/usr/bin/env python3
"""Stop hook — не дать ответить канцеляритом на разговорный заход хозяина.

хозяин, 10.08.2026: «сколько раз я просил — и что толку. Тебя хватает на три
...
Гейт: заход хозяина разговорный (мат/сленг), ответ длинный, а в ответе ни одного
...
def turn(transcript: Path) -> tuple[str, str]:
    """(последнее сообщение хозяина; всё, что я ему отправил в этом ходе)."""
    rows = []
...
def verdict(asked: str, said: str) -> str | None:
    ...
    if CASUAL_OUT.search(said):
        return None
    return (
        "хозяин зашёл в разговорном регистре, а ответ сухой и official-looking"
